...
import torch
import numpy as np
from hugs.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation
from torch import nn
import os
from hugs.utils.system_utils import mkdir_p
from plyfile import PlyData, PlyElement
from hugs.utils.sh_utils import RGB2SH
from simple_knn._C import distCUDA2
from hugs.utils.graphics_utils import BasicPointCloud
from hugs.utils.general_utils import strip_symmetric, build_scaling_rotation
import trimesh
from hugs.utils.vis_utils import save_points
...
class GaussianModel:
    ...
    def restore(self, state_dict, cfg):
        ...
        denom,
        opt_dict, 
        self.spatial_lr_scale) = state_dict
    ...
    @get_opacity.setter
    def get_opacity(self, new_opacity):
    ...
    def setup_optimizer(self, cfg):
        ...
        l = [
            ...
            {'params': [self._appearance_embeddings], 'lr': cfg.appearance_embeddings_lr, "name": "appearance_embeddings"},
            {'params': self.appearance_network.parameters(), 'lr': cfg.appearance_network_lr, "name": "appearance_network"}
        ]
    ...
    # def load_ply(self, path): # TODO: remove later; automate manual changes form Meshlab
    #     plydata = PlyData.read(path)
        
    #     # NOTE: read original output pointcloud as well; contains more information
    #     # meshlab somehow lost most point attributes... 
    #     path_copy = 'test_colmap/gof/point_cloud/iteration_3000/point_cloud_copy.ply' 
    #     plydata_copy = PlyData.read(path_copy)
    #     opacities_copy = np.asarray(plydata_copy.elements[0]["opacity"])
    #     filter_3D_copy = np.asarray(plydata_copy.elements[0]["filter_3D"])
    #     f_dc_0_copy = np.asarray(plydata_copy.elements[0]["f_dc_0"])
    #     f_dc_1_copy = np.asarray(plydata_copy.elements[0]["f_dc_1"])
    #     f_dc_2_copy = np.asarray(plydata_copy.elements[0]["f_dc_2"])
    #     xyz_copy = np.stack((   np.asarray(plydata_copy.elements[0]["x"]),
    #                             np.asarray(plydata_copy.elements[0]["y"]),
    #                             np.asarray(plydata_copy.elements[0]["z"])),  axis=1)
    ...
    #     tree = cKDTree(xyz_copy)
    #     distances, indices = tree.query(xyz)
    #     opacities = opacities_copy[indices][..., np.newaxis]
    #     assert np.all(distances == 0), "Some points do not match exactly."

    #     opacities = opacities_copy[indices][..., np.newaxis]
    #     filter_3D = filter_3D_copy[indices][..., np.newaxis]    
    #     f_dc_0 = f_dc_0_copy[indices]
    #     f_dc_1 = f_dc_1_copy[indices]
    #     f_dc_2 = f_dc_2_copy[indices]

    #     features_dc = np.zeros((xyz.shape[0], 3, 1))
    #     features_dc[:, 0, 0] = f_dc_0.squeeze()
    #     features_dc[:, 1, 0] = f_dc_1 
    #     features_dc[:, 2, 0] = f_dc_2 

    #     extra_f_names = [p.name for p in plydata_copy.elements[0].properties if p.name.startswith("f_rest_")]
    #     extra_f_names = sorted(extra_f_names, key = lambda x: int(x.split('_')[-1]))
    #     assert len(extra_f_names)==3*(self.max_sh_degree + 1) ** 2 - 3
    #     features_extra = np.zeros((xyz.shape[0], len(extra_f_names)))
    #     for idx, attr_name in enumerate(extra_f_names):
    #         feat_copy = np.asarray(plydata_copy.elements[0][attr_name])
    #         matching_feat = feat_copy[indices]
    #         features_extra[:, idx] = matching_feat 
    #     # Reshape (P,F*SH_coeffs) to (P, F, SH_coeffs except DC)
    #     features_extra = features_extra.reshape((features_extra.shape[0], 3, (self.max_sh_degree + 1) ** 2 - 1))

    #     scale_names = [p.name for p in plydata_copy.elements[0].properties if p.name.startswith("scale_")]
    #     scale_names = sorted(scale_names, key = lambda x: int(x.split('_')[-1]))
    #     scales = np.zeros((xyz.shape[0], len(scale_names)))
    #     for idx, attr_name in enumerate(scale_names):
    #         scales_copy = np.asarray(plydata_copy.elements[0][attr_name])
    #         matching_scale = scales_copy[indices]
    #         scales[:, idx] = np.asarray(matching_scale)

    #     rot_names = [p.name for p in plydata_copy.elements[0].properties if p.name.startswith("rot")]
    #     rot_names = sorted(rot_names, key = lambda x: int(x.split('_')[-1]))
    #     rots = np.zeros((xyz.shape[0], len(rot_names)))
    #     for idx, attr_name in enumerate(rot_names):
    #         rot_copy = np.asarray(plydata_copy.elements[0][attr_name])
    #         matching_rot = rot_copy[indices]
    #         rots[:, idx] = np.asarray(matching_rot)
