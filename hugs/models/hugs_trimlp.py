...
class HUGS_TRIMLP:
    ...
    def __init__(
        ...
        self.deformation_dec = DeformationDecoder(
            n_features=n_features * 3, disable_posedirs=disable_posedirs
        ).to("cuda")
        self.geometry_dec = GeometryDecoder(
            n_features=n_features * 3, use_surface=use_surface
        ).to("cuda")

        if n_subdivision > 0:
            logger.info(f"Subdividing SMPL model {n_subdivision} times")
            self.smpl_template = subdivide_smpl_model(
                smoothing=True, n_iter=n_subdivision
            ).to(self.device)
        else:
    ...
    def forward_test(
        ...
        if self.isotropic:
            gs_scales = torch.ones_like(gs_scales) * torch.mean(
                gs_scales, dim=-1, keepdim=True
            )
    ...
    def setup_optimizer(self, cfg):
        ...
        params = [
            {
                "params": [self._xyz],
                "lr": cfg.position_init * cfg.smpl_spatial,
                "name": "xyz",
            },
